...
/** @brief  Sector 1 读写缓冲区（BspFlash_WriteConfig / ClearTrace 共享） */
static uint8_t s_sector1_buf[SECTOR1_USED_SIZE];
...
int BspFlash_WriteConfig(const ota_config_t *p_cfg)
{
    ...
    memcpy(s_sector1_buf, (const void *)FLASH_ADDR_CONFIG, SECTOR1_USED_SIZE);

    /* 只更新 Config 部分，Trace 和 Crash Dump 保持原样 */
    memcpy(s_sector1_buf, &tmp, sizeof(ota_config_t));
    ...
    ret = BspFlash_Write(FLASH_ADDR_CONFIG, s_sector1_buf, SECTOR1_USED_SIZE);
    if (ret != 0)
    {
        return -1;
    }
...
int BspFlash_ClearTrace(void)
{
    ...
    memset(s_sector1_buf + (OTA_TRACE_ADDR - FLASH_ADDR_CONFIG),
           0xFF,
    ...
    if (BspFlash_Write(FLASH_ADDR_CONFIG, s_sector1_buf, SECTOR1_USED_SIZE) != 0)
    {
        return -1;
    }
